...
Dip-confirm logic:
  ...
  2. Between bars 1 and check_bars-1 the price must drop ≥ dip_thresh below p0
  3. At check_bars the price must have recovered ≥ recovery_floor × p0
     (default recovery_floor=0.95 — doesn't need full recovery, just most of it)
  4. If triggered: enter at path[check_bars] + haircut, hold to resolution

--fidelity controls resolution of dip detection:
  1440 (default) : daily closes — original methodology
  60             : hourly bars — matches live hourly polling
  1              : 1-minute bars — matches live 1-min history fetch (slow)

check_day is always in calendar days; internally converted to bars.

Multiple horizons: [5, 7, 10, 14, 21] days
Multiple dip thresholds: [0.05, 0.10, 0.15, 0.20]
check_day fixed at 2 (observe dip, enter on day 2 recovery)

Usage
-----
  python experiments/run_dip_confirm_backtest.py
  python experiments/run_dip_confirm_backtest.py --fidelity 60
  python experiments/run_dip_confirm_backtest.py --fidelity 60 --n-markets 500
"""
import argparse
import logging
import time
...
from posterioralpha.polymarket.fetch import (
    fetch_markets, fetch_token_history, fetch_token_history_raw,
)
...
def build_paths(markets, H, lo, hi, sleep, fidelity_minutes=1440):
    """Build price paths for every market in [lo,hi) at H days to resolution.

    fidelity_minutes controls bar width:
      1440 → one bar per calendar day (original methodology)
      60   → one bar per hour (matches live hourly min-tracking)
      1    → one bar per minute (matches live 1-min fetch; slow for large n)
    """
    bars_per_day = max(1, 1440 // fidelity_minutes)
    freq = f"{fidelity_minutes}min" if fidelity_minutes < 1440 else "D"
    paths = []
    for i, m in enumerate(markets.itertuples(index=False), 1):
        try:
            if fidelity_minutes == 1440:
                ...
                s = fetch_token_history_raw(m.yes_token, fidelity_minutes=fidelity_minutes,
                                            use_cache=True)
                if s.empty:
                    continue
                # Align to the fidelity grid anchored at resolution
                t_res = s.index.max()
                t_entry = t_res - pd.Timedelta(days=H)
                grid = pd.date_range(t_entry, t_res, freq=freq, tz="UTC")
                sp = s.reindex(s.index.union(grid)).sort_index().ffill().reindex(grid).dropna()

            min_len = H * bars_per_day + 1
            if len(sp) < min_len:
                continue
            sp = sp.iloc[-min_len:]
            p0 = float(sp.iloc[0])
            ...
            logger.debug("skip %s: %s", m.yes_token[:12], exc)
            continue

        if i % 50 == 0:
            logger.info("  H=%dd fid=%dmin: built %d/%d (%d in band)",
                        H, fidelity_minutes, i, len(markets), len(paths))
        if sleep and i % 5 == 0:
            time.sleep(sleep)
    return paths


def classify(path, p0, check_day, dip_thresh, bars_per_day=1, recovery_floor=0.95):
    """Return (label, conf_price).

    check_day is in calendar days; converted to bars using bars_per_day.
    Intra window = bars 1 .. check_bars-1 (exclusive); recovery at check_bars.
    """
    check_bars = check_day * bars_per_day
    if len(path) <= check_bars:
        return "short", None
    intra  = path[1:check_bars]
    dipped = len(intra) > 0 and any(px < p0 * (1.0 - dip_thresh) for px in intra)
    conf_px = path[check_bars]
...
def main() -> None:
    ...
    ap.add_argument("--check-day",      type=int,   default=2,
                    help="calendar days after watch to check for recovery")
    ap.add_argument("--recovery-floor", type=float, default=0.95)
    ap.add_argument("--fidelity",       type=int,   default=1440,
                    help="bar width in minutes: 1440=daily (original), 60=hourly, 1=1-min (slow)")
    ...
    CD  = args.check_day
    FID = args.fidelity
    bars_per_day = max(1, 1440 // FID)

    fid_label = {1440: "daily closes", 60: "hourly bars", 1: "1-min bars"}.get(FID, f"{FID}min bars")
...
╚══════════════════════════════════════════════════════════╝
  band [{lo:.2f},{hi:.2f})  ·  check_day={CD}d  ·  recovery_floor={args.recovery_floor:.0%}
  fidelity: {FID}min ({fid_label}) — dip measured over {bars_per_day} bars/day
  horizons: {HORIZONS}  ·  dip thresholds: {[f"{t:.0%}" for t in DIP_THRESHOLDS]}
""")
    ...
    for H in HORIZONS:
        logger.info("Building paths H=%dd fidelity=%dmin ...", H, FID)
        paths = build_paths(markets, H, lo, hi, args.sleep, fidelity_minutes=FID)
        if not paths:
        ...
        for thresh in DIP_THRESHOLDS:
            ...
            for p in paths:
                label, conf_px = classify(
                    p["path"], p["entry_price"], CD, thresh,
            ...
            rows.append({
                "H": H, "thresh": thresh, "fidelity": FID, "n_all": n_all,
                "base_wr": round(base_wr, 3),
                "n_dip_recover": int((labels == "dip_recover").sum()),
                "n_dip_persist": int((labels == "dip_persist").sum()),
                "n_steady":      int((labels == "steady").sum()),
    ...
    # ── Print results ─────────────────────────────────────────────────────────
    print(f"\n  Fidelity: {fid_label} ({FID}min bars, {bars_per_day} bars/day)\n")
    for thresh in DIP_THRESHOLDS:
        ...
        print(f"  {'-'*4}-+-{'-'*6}-+-{'-'*6}-+-{'-'*5}-+-{'-'*8}-+-{'-'*13}-+-"
              f"{'-'*8}-+-{'-'*7}-+-{'-'*7}-+-{'-'*6}")
        for _, r in sub.iterrows():
            ci_str = f"{r['dc_ci_lo']:.0%}–{r['dc_ci_hi']:.0%}" if r["dc_n"] >= 3 else "too few"
            ...
            print(f"  {int(r['H']):>4} | {int(r['n_all']):>6} | {r['base_wr']:>5.0%} | "
                  ...
                  f"{r['dc_mean']:>+7.1%} | {r['dc_sh']:>7.2f} | {r['flat_H_sh']:>7.2f} | "
                  f"{dsh:>+5.2f}")

    # ── Plot ──────────────────────────────────────────────────────────────────
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(
        f"Dip-confirm — band [{lo:.2f},{hi:.2f}), check_day={CD}, fidelity={fid_label}",
