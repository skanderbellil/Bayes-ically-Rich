...
from posterioralpha.research import intramonth_window_mask, wml_formation
...
def label_window(holding_days: pd.DatetimeIndex) -> np.ndarray:
    """Thin wrapper over the framework primitive, pinned to this study's
    T-WINDOW_START … T-WINDOW_END window (see posterioralpha.research.intramonth)."""
    return intramonth_window_mask(holding_days, start=WINDOW_START, end=WINDOW_END)
